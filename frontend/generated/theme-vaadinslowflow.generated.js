...
import stylesCss from 'themes/vaadinslowflow/styles.css';
...
window.Vaadin['_vaadintheme_vaadinslowflow_globalCss'] = window.Vaadin['_vaadintheme_vaadinslowflow_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_vaadinslowflow_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_vaadinslowflow_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_vaadinslowflow_globalCss'].push(target);
  }
  if (!document['_vaadintheme_vaadinslowflow_componentCss']) {
    
    document['_vaadintheme_vaadinslowflow_componentCss'] = true;
  }
